...
test('employee still submits pending wfh requests and cannot use super admin self routes', function () {
    ...
    $this->actingAs($employee->user)
        ->post('/attendance/wfh', [
            'start_date' => $start,
            'end_date' => $start,
            'reason' => 'Need to stay home.',
        ])
        ->assertRedirect()
        ->assertSessionHas('success', 'WFH request submitted.');
...
test('operations user with manage wfh cannot record via super admin direct path semantics', function () {
    ...
    $this->actingAs($ops->user)
        ->post('/attendance/wfh', [
            'start_date' => $start,
            'end_date' => $start,
            'reason' => 'Ops self request stays pending.',
        ])
        ->assertRedirect();

    $record = WfhRequest::query()->where('employee_id', $ops->id)->sole();

    expect($record->type)->toBe(WfhRequestType::Request)
        ->and($record->status)->toBe(WfhRequestStatus::Pending);
});
